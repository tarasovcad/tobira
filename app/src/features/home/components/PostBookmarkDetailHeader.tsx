...
export function PostBookmarkDetailHeader({onBack}: PostBookmarkDetailHeaderProps) {
  return (
    <header className="border-border flex shrink-0 items-center gap-3 stroke-1 px-6 pt-3.5 pb-5">
      <PostBookmarkDetailBackButton onBack={onBack} />
      <h1 className="text-foreground/95 text-[17px] font-semibold">{POST_DETAIL_TITLE}</h1>
    </header>
  );
}
