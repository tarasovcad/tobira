...
export function SidebarMain({
  ...
  const settingsButtonClassName = cn(
    "flex w-full items-center rounded-md text-sm font-medium transition-[padding] duration-50 ease-linear",
    "text-secondary hover:bg-muted hover:text-foreground cursor-pointer bg-transparent",
    ...
    isCollapsed ? "justify-start px-2 py-2" : "justify-start px-3 py-2",
    pathname === "/settings" && "text-foreground bg-muted-strong",
  );
    ...
    <>
      ...
      <span
        className={cn(
          ...
          isCollapsed ? "ml-0 max-w-0 opacity-0" : "ml-2 opacity-100",
        )}>
        Settings
      </span>
    </>
      ...
      <div className={cn("shrink-0 p-3 pt-0")}>
        ...
        {isCollapsed ? (
          <TooltipTrigger
            ...
            payload={SettingsTooltipContent}
            render={
              <button
                type="button"
                onClick={() => {
